...
import { buildResponseEventErrorFor, GenericErrorType, EventMessage, buildNoEventHandlerFor, buildBadProtocolFor } from "./responseEventBuilder";

export class EventProcessor {

    static eventDiscovery: Map<string, (event: Event) => Promise<Event>> = new Map<string, (event: Event) => Promise<Event>>();
    
    static addHandler(eventName: string, eventVersion: number, handler : (event: Event) => Promise<Event> ) {
    ...
    static processEvent(rawEvent: any) : Promise<Event> {
        ...
        if (hanldlerFunction) {
            ...
            .catch(err => {
                ...
                return new Promise<Event>((resolve, _) => {
                    resolve(buildResponseEventErrorFor(event, GenericErrorType, payloadUnhandledErrorMessage));
                })
            })
        }else{
            return new Promise<Event>((resolve, _) => {
    ...
    private static validateEvent(rawEvent: any) {
