...
const mockHandlerFunction = (event: Event) => Promise.resolve(buildResponseEventFor(intoEvent(rawEvent)))
...
describe("Test xray instrumentation",() => {
    ...
    test("When dependency is found should call instrumentation", async () => {
        ...
        //Execute
        const responseEvent = await instrumentExecutionOnXray(intoEvent(rawEvent), mockHandlerFunction)
    ...
    test("When dependency is not found should not call instrumentation", async () => {
        //Setup
        //const XRAY = require("aws-xray-sdk")
        jest.mock("aws-xray-sdk",() => {throw new Error()})
